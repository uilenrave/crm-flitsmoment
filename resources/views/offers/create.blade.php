...
@section('actions')
    <a href="{{ route('leads.show', $lead) }}" class="btn btn-secondary">← Terug naar lead</a>
@endsection
    ...
    <div style="display:grid;grid-template-columns:1fr 340px;gap:1.25rem;align-items:start;">
        ...
        <div>
            ...
            <div class="card" style="margin-bottom:1.25rem;">
                ...
                <div class="form-grid" style="padding:1rem;">
                    <div style="grid-column:span 2;">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', 'Offerte Photobooth') }}" required style="width:100%;padding:.5rem .75rem;border:1px solid #e2e8f0;border-radius:.375rem;font-size:.875rem;">
                    </div>
                    <div>
                        <label for="expires_at">Geldig tot</label>
                        <input type="date" id="expires_at" name="expires_at" value="{{ old('expires_at', now()->addDays($expiryDays)->format('Y-m-d')) }}" style="width:100%;padding:.5rem .75rem;border:1px solid #e2e8f0;border-radius:.375rem;font-size:.875rem;">
                    </div>
                </div>
            </div>
                ...
                <div style="padding:.75rem 1rem;border-top:1px solid #e2e8f0;">
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem;">
                        ...
                        <div style="display:flex;align-items:center;gap:.25rem;">
                            ...
                            <input type="number" id="discount-input" step="0.01" min="0" value="{{ old('discount', 0) }}" style="width:100px;padding:.375rem .5rem;border:1px solid #e2e8f0;border-radius:.375rem;font-size:.875rem;text-align:right;" onchange="recalcTotals()">
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div style="padding:1rem;">
                    <textarea name="terms_text" rows="8" style="width:100%;padding:.5rem .75rem;border:1px solid #e2e8f0;border-radius:.375rem;font-size:.8rem;resize:vertical;line-height:1.6;">{{ old('terms_text', $defaultTerms) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="card" style="margin-bottom:1rem;">
                <div class="card-header"><span class="card-title">Lead gegevens</span></div>
                <div style="font-size:.8rem;">
                    ...
                    <div style="padding:.5rem 0;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;">
                        <span style="color:#64748b;">Telefoon</span>
                        <span>{{ $lead->phone ?? '—' }}</span>
                    </div>
                    <div style="padding:.5rem 0;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;">
                        <span style="color:#64748b;">Eventdatum</span>
                        <span>{{ $lead->event_date?->format('d M Y') ?? '—' }}</span>
                    </div>
                    <div style="padding:.5rem 0;display:flex;justify-content:space-between;">
                        <span style="color:#64748b;">Locatie</span>
...
@push('scripts')
<script>
    const initialItems = @json($lineItems);
    let rowIndex = 0;
